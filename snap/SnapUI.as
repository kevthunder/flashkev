package {
	...
	public class SnapUI extends UIComponent{
		//////////// variables ////////////
		...
		public function curRectAsOrigin(){
			...
			originRect = new Rectangle(x,y,width,height);
			originPos = new Point(x,y);
			//trace(originRect);
		}
		public function update(){
			if(_snapEnabled){
				var root = this.root as ResizableRoot;
				if(root!=null && snapOptions.length && originRect != null){
					...
					if(snapRect!=null){
						...
						updatedRect = new Rectangle(x,y,width,height);
					}
					...
					if(individualDim){
						if(xSnapOptions){
							x = calculateDim(xSnapOptions,root.stageRect.x, root.stageRect.width, originPos.x, root.originalStageRect.x, root.originalStageRect.width);
						}
						if(ySnapOptions){
							y = calculateDim(ySnapOptions,root.stageRect.y, root.stageRect.height, originPos.y, root.originalStageRect.y, root.originalStageRect.height);
						}
						if(widthSnapOptions){
							width = calculateDim(widthSnapOptions, 0, root.stageRect.width, originRect.width, 0, root.originalStageRect.width);
						}
						if(heightSnapOptions){
							height = calculateDim(heightSnapOptions, 0, root.stageRect.height, originRect.height, 0, root.originalStageRect.height);
						}
					}
				}
			}
